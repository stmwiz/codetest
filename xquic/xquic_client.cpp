...
int
read_file_data(char *data, size_t data_len, const char *filename)
{
    int ret = 0;
    ...
    struct sockaddr **saddr, socklen_t *saddr_len)
{
    if (type == AF_INET6) {
        ...
        *saddr = (struct sockaddr *)calloc(1, sizeof(struct sockaddr_in));
    ...
    char *path_interface, user_conn_t *user_conn)
{
    ...
    path->peer_addr = (struct sockaddr    *)calloc(1, user_conn->peer_addrlen);
...
xqc_client_request_send(xqc_h3_request_t *h3_request, user_stream_t *user_stream)
{
    ...
    xqc_http_header_t header[MAX_HEADER] = {
        ...
        {
            .name   = {.iov_base = (void*)"host", .iov_len = 4},
        ...
        {
            .name   = {.iov_base = (void*)"content-type", .iov_len = 12},
            .value  = {.iov_base = (void*)"text/plain", .iov_len = 10},
            .flags  = 0,
        },
        {
            .name   = {.iov_base = (void*)"content-length", .iov_len = 14},
    ...
    if (g_test_case == 29) {
        ...
        xqc_http_header_t test_long_hdr = {
            .name   = {.iov_base = (void*)"long_filed_line", .iov_len = 15},
    ...
    if (g_test_case == 34) {
        ...
        xqc_http_header_t uppercase_name_hdr = {
            .name   = {.iov_base = (void*)"UpperCaseFiledLineName", .iov_len = 22},
            .value  = {.iov_base = (void*)"UpperCaseFiledLineValue", .iov_len = 23},
            .flags  = 0,
        };
        ...
        xqc_http_header_t lowcase_start_hdr = {
            .name   = {.iov_base = (void*)"filelineNamewithLowerCaseStart", .iov_len = 30},
            .value  = {.iov_base = (void*)"UpperCaseFiledLineValue", .iov_len = 23},
            .flags  = 0,
        };
        ...
        xqc_http_header_t test_long_hdr = {
            ...
            .value  = {.iov_base = (void*)"header_with_long_name", .iov_len = 21},
            .flags  = 0,
        };
    ...
    /* send body */
    if (user_stream->send_offset < user_stream->send_body_len) {
        ret = xqc_h3_request_send_body(h3_request, (unsigned char *)user_stream->send_body + user_stream->send_offset, user_stream->send_body_len - user_stream->send_offset, fin);
        if (ret == -XQC_EAGAIN) {
            return 0;
...
xqc_client_request_close_notify(xqc_h3_request_t *h3_request, void *user_data)
{
    ...
    if (g_req_cnt < g_req_max) {
        user_stream = (user_stream_t *)calloc(1, sizeof(user_stream_t));
...
#define TIMEOUT 10
        ...
        do {
            ...
            for (int i = 0; i < retval; i++) {
                ...
                if (xqc_engine_packet_process(ctx.engine, (const unsigned char*)iovecs[i].iov_base, msgs[i].msg_len,
...
xqc_client_write_log(xqc_log_level_t lvl, const void *buf, size_t count, void *engine_user_data)
{
    ...
    int log_len = ::snprintf((char*)log_buf, XQC_MAX_LOG_LEN + 1, "%s\n", (char*)buf);
    if (log_len < 0) {
